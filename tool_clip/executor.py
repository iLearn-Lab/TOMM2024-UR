...
class Executor:
    ...
    def __call__(self, caption: str, image: Image, boxes: List[Box], image_name: str = None) -> torch.Tensor:
        ...
        for model, images_t, model_name in zip(self.models, images, self.model_names):
            ...
            all_logits_per_image.append(logits_per_image) #(2, img_num, txt_num)
        ...
        all_logits_per_image = torch.stack(all_logits_per_image).sum(0) #(img_num, txt_num)
        ...
        if self.method_aggregator == "max":
            ...
            all_logits_per_image = all_logits_per_image.view(2, len(boxes), len(caption)).sum(dim=0, keepdim=True)
        return all_logits_per_image.squeeze(0)

class ClipExecutor(Executor):
    def __init__(self, clip_model: str = "ViT-B/32", device: str = "cuda:0", box_representation_method: str = "crop", method_aggregator: str = "max", enlarge_boxes: int = 0, expand_position_embedding: bool = False, square_size: bool = False, blur_std_dev: int = 100, cache_path: str = None) -> None:
    ...
    def preprocess_text(self, text: str) -> torch.Tensor:
        ...
        return clip.tokenize(["a photo of "+txt for txt in text])
    ...
    def call_model(self, model: torch.nn.Module, images: torch.Tensor, text: torch.Tensor, image_features: torch.Tensor = None, text_features: torch.Tensor = None) -> torch.Tensor:
        if image_features is None:
            print('computing image features33')
