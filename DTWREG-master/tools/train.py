...
def main(args):
    ...
    # set up loader
    data_json = osp.join('/home/share2/zpp/MAttNet/cache/prepro', opt['dataset_splitBy'], 'data.json')
    data_h5 = osp.join('/home/share2/zpp/MAttNet/cache/prepro', opt['dataset_splitBy'], 'data.h5')
    #sub_obj_wds = osp.join('/home/share2/zpp/ARN-master/kprn/cache/sub_obj_wds', opt['dataset_splitBy'], 'sub_obj_wds.json')
    #similarity = osp.join('/home/share2/zpp/ARN-master/kprn/cache/similarity', opt['dataset_splitBy'], 'similarity.json')
    sent_extract = json.load(open(osp.join('cache/sub_obj_wds', opt['dataset_splitBy'], "sent_extract.json")))
    loader = DataLoader(data_h5=data_h5, data_json=data_json, sub_obj_wds = None, similarity=None, opt=opt, sent_extract=sent_extract)

    # prepare feats
    feats_dir = '%s_%s_%s' % (args.net_name, args.imdb_name, args.tag)
    head_feats_dir = osp.join('/home/share2/zpp/MAttNet/cache/feats/', opt['dataset_splitBy'], 'mrcn', feats_dir)

    loader.prepare_mrcn(head_feats_dir, args)

    ann_feats = osp.join('/home/share2/zpp/MAttNet/cache/feats', opt['dataset_splitBy'], 'mrcn',
    ...
    while True:
        ...
        anns_words = data['anns_words']
        sub_words = data['sub_words']
        obj_words = data['obj_words']
        img_ref_ids = data['img_ref_ids']
        img_sent_ids = data['img_sent_ids']
        gd_ixs = data['gd_ixs']
        ...
        scores, loss, sub_loss, obj_loss, rel_loss = model(Feats['pool5'], Feats['fc7'], Feats['lfeats'], Feats['dif_lfeats'], Feats['cxt_fc7'],
                             ...
                             ann_pool5, ann_fc7, ann_fleats, anns_glove_emb)
        try:
          loss.backward()
        ...
        if opt['learning_rate_decay_start'] > 0 and iter > opt['learning_rate_decay_start']: #从30000开始变化
