...
class PairEncoder(nn.Module):
    ...
    def forward(self, pool5, fc7, ann_pool5, ann_fc7, ann_fleats, anns_glove_emb):
        sent_num, ann_num, grids = pool5.size(0), pool5.size(1), pool5.size(3) * pool5.size(4)
        batch = sent_num * ann_num
        ...
        expand_1_pool5 = ann_pool5.unsqueeze(1).expand(ann_num, ann_num, self.pool5_dim)  #(ann_num, ann_num, dim)
        expand_0_pool5 = ann_pool5.unsqueeze(0).expand(ann_num, ann_num, self.pool5_dim)  #(ann_num, ann_num, dim)

        expand_1_pool5 = expand_1_pool5.contiguous().view(-1, self.pool5_dim)             #(ann_num*ann_num, dim)
        expand_0_pool5 = expand_0_pool5.contiguous().view(-1, self.pool5_dim)             #(ann_num*ann_num, dim)
        
        ##(sent_num, ann_num*ann_num, dim)
        ...
        ann_fleats = self.fc(ann_fleats)                                #(ann_num, 512)
